...
(Fonction (globale, usine) {
	...
	if (typeof module === "objet" && typeof module.exports === "objet") {
		...
		// est présent, exécute l'usine et obtient jQuery.
		...
		// par exemple var jQuery = require ("jquery") (fenêtre);
...
// Passer ceci si la fenêtre n'est pas encore définie
}) (Fenêtre typeof! == "non défini"? window: this, function (window, noGlobal) {

// Edge <= 12 - 13+, Firefox <= 18 - 45+, IE 10 - 11, Safari 5.1 - 9+, iOS 6 - 9.1
// lance des exceptions lorsque le code non strict (par exemple, ASP.NET 4.5) accède au mode strict
// arguments.callee.caller (trac-13335). Mais à partir de jQuery 3.0 (2016), le mode strict devrait être commun
// Assez que toutes ces tentatives soient gardées dans un bloc try.
"use strict";
...
var isFunction = fonction isFunction (obj) {
      ...
      // (ie, typeof document.createElement ("objet") === "fonction" `).
      // Nous ne voulons pas classer * n'importe quel noeud DOM en tant que fonction.
...
var isWindow = fonction isWindow (obj) {
	...
	fonction DOMEval (code, doc, noeud) {
		...
		script.text = code;
		if (noeud) {
			pour (i dans preservScriptAttributes) {
				if (noeud [i]) {
					script [i] = noeud [i];
				}
			}
...
function toType (obj) {
	...
	// Support: Android <= 2.3 seulement (RegExp fonctionnel)
...
// non protégé dans un autre endroit, il semble plus sûr de définir global uniquement pour ce module
...
var
	...
	jQuery = fonction (sélecteur, contexte) {
		...
		// L'objet jQuery est en fait juste le constructeur init 'enhanced'
		// Nécessite init si jQuery est appelé (il suffit d'autoriser l'erreur si elle n'est pas incluse)
		return new jQuery.fn.init (sélecteur, contexte);
	},

	// Support: Android <= 4.0 seulement
...
jQuery.fn = jQuery.prototype = {
	...
	// Récupère le Nième élément de l'ensemble d'éléments correspondant OU
	// Récupère l'ensemble des éléments correspondants en tant que tableau propre
	get: function (num) {
		...
		renvoie num <0? ceci [num + this.length]: ceci [num];
	},
	...
	pushStack: function (elems) {
		...
		// Construit un nouvel ensemble d'éléments correspondant à jQuery
	...
	each: function (callback) {
		renvoie jQuery.each (this, callback);
	},
...
jQuery.extend = jQuery.fn.extend = function () {
	...
	if (typeof target === "booléen") {
		...
		// Passer le booléen et la cible
		target = arguments [i] || {};
		i ++;
	}

	// Gère le cas où la cible est une chaîne ou quelque chose (possible en copie profonde)
	if (typeof target! == "objet" &&! isFunction (cible)) {
		target = {};
	}

	// Etendre jQuery lui-même si un seul argument est passé
	si (i === longueur) {
		target = ceci;
		je--;
	}
...
jQuery.extend ({
	...
	isPlainObject: function (obj) {
		...
		// Utilise toString au lieu de jQuery.type pour attraper les objets hôtes
		if (! obj || toString.call (obj)! == "[objet d'objet]") {
			return false;
		}
		...
		// Les objets avec le prototype sont simples si elles ont été construites par une fonction Object globale
	...
	isEmptyObject: function (obj) {
		...
		/ * eslint-disable non-inutilisé-vars * /
	...
	// les résultats sont pour usage interne seulement
	...
	fusionner: fonction (première, seconde) {
		var len = + seconde.length,
			j = 0,
			i = première longueur;
	...
	grep: function (elems, callback, invert) {
		...
		pour (; i <longueur; i ++) {
			callbackInverse =! callback (élémets [i], i);
	...
	map: function (elems, callback, arg) {
		...
		// Parcourt le tableau, en traduisant chacun des éléments à leurs nouvelles valeurs
...
if (typeof Symbole === "fonction") {
	jQuery.fn [Symbol.iterator] = arr [Symbol.iterator];
}

// Remplit la carte class2type
jQuery.each ("Chaîne de nombre booléenne Fonction tableau date RegExp objet erreur symbole" .split (""),
function (i, nom) {
	class2type ["[objet" + nom + "]"] = name.toLowerCase ();
});
...
function isArrayLike (obj) {
	...
	// concernant la longueur Nodelist dans IE
	...
	return type === "tableau" || longueur === 0 ||
...
var i,
	...
	push = arr.push,
	tranche = arr.slice,
	// Utiliser un indexOf réduit car il est plus rapide que natif
	...
	booleans = "checked | selected | async | autofocus | autoplay | controls | diffère | disabled | caché | ismap | loop | multiple | ouvert | readonly | requis | scoped",
	...
	attributes = "\\ [" + espace + "* (" + identifiant + ") (?:" + espace +
		// Opérateur (capture 2)
		"* ([* ^ $ |! ~]? =)" + espace +
	...
	pseudos = ":(" + identifiant + ") (?: \\ ((" +
		// Pour réduire le nombre de sélecteurs requis dans le préfixe, préférez les arguments:
	...
	// Espace de début et d'échappement non-échappé, capturant des caractères non-blancs précédant ce dernier
	...
	matchExpr = {
		...
		"CHILD": nouveau RegExp ("^ :( seulement | premier | dernier | nième | nième-dernier) - (enfant | de-type) (?: \\ (" + espace +
			"* (pair | impair | (([+ -] |) (\\ d *) n |)" + espace + "* (?: ([+ -] |)" + espace +
			"* (\\ d +) |))" + espace blanc + "* \\) |)", "i"),
		"bool": nouveau RegExp ("^ (?:" + booléens + ") $", "i"),
		// A utiliser dans les bibliothèques implémentant .is ()
		// Nous utilisons ceci pour la correspondance de POS dans `select`
		"needsContext": new RegExp ("^" + espace blanc + "* [> + ~] |: (pair | impair | eq | gt | lt | nième | premier | dernier) (?: \\ (" +
			espace blanc + "* ((?: - \\ d)? \\ d *)" + espace + "* \\) |) (? = [^ -] | $)", "i")
	},

	rinputs = / ^ (?: input | select | textarea | bouton) $ / i,
	rheader = / ^ h \ d $ / i,

	rnative = / ^ [^ {] + \ {\ s * \ [natif \ w /,

	// ID facilement consultable / récupérable ou sélecteurs TAG ou CLASS
	rquickExpr = /^(?:#([\w-]+)|(\w+)|\.([\w-]+))$/,
	...
	funescape = function (_, escapeed, escapedWhitespace) {
		var high = "0x" + échappé - 0x10000;
		// NaN signifie non-codepoint
		// Support: Firefox <24
		// Solution de contournement de l'interprétation numérique erronée de + "0x"
		retour haut! == haut || escapedWhitespace?
			échappé :
			haut <0?
				// Point de code BMP
				String.fromCharCode (high + 0x10000):
				// Codepoint du plan supplémentaire (paire de substitution)
				String.fromCharCode (haut >> 10 | 0xD800, haut & 0x3FF | 0xDC00);
	},

	// Sérialisation de chaîne / identificateur CSS
	...
	fcssescape = function (ch, asCodePoint) {
		if (asCodePoint) {
			...
			// Les caractères de contrôle et les nombres (dépendants de la position) sont échappés en tant que points de code
			return ch.slice (0, -1) + "\\" + ch.charCodeAt (ch.length - 1) .toString (16) + "";
		}

		// D'autres caractères ASCII potentiellement spéciaux sont protégés par un backslash
		retourne "\\" + ch;
	},
	...
	disabledAncestor = addCombinator (
		function (elem) {
			return elem.disabled === true && ("form" dans elem || "label" dans elem);
		},
		{dir: "parentNode", ensuite: "legend"}
	)
...
essayez {
	...
	// Support: Android <4.0
	// Détecter l'échec de push.apply en mode silencieux
	arr [preferredDoc.childNodes.length] .nodeType;
} catch (e) {
	pousser = {appliquer: arr.length?
		...
		fonction (cible, els) {
			var j = target.length,
				i = 0;
			// Impossible d'approuver NodeList.length
			while ((cible [j ++] = els [i ++])) {}
			target.length = j - 1;
		}
	}
...
fonction Sizzle (sélecteur, contexte, résultats, graine) {
	var m, i, elem, nid, correspondance, groupes, newSelector,
		...
		// nodeType par défaut à 9, car le contexte par défaut est document
		nodeType = contexte? context.nodeType: 9;

	résultats = résultats || [];

	// Retour anticipé des appels avec un sélecteur ou un contexte non valide
	...
	if (! seed) {
		...
		if ((contexte? context.ownerDocument || contexte: preferredDoc)! == document) {
		...
		if (documentIsHTML) {
			...
			// (excepté le contexte DocumentFragment, où les méthodes n'existent pas)
			if (nodeType! == 11 && (match = rquickExpr.exec (selector))) {

				// Sélecteur d'identifiant
				if ((m = correspondance [1])) {
					...
					if (nodeType === 9) {
						if ((elem = context.getElementById (m))) {
							...
							// getElementById peut correspondre aux éléments par nom au lieu de l'ID
							if (elem.id === m) {
								results.push (elem);
								renvoyer les résultats;
							}
				...
				// Sélecteur de classe
				} else if ((m = correspondance [3]) && support.getElementsByClassName &&
			...
			if (support.qsa &&
				...
				// Exclure les éléments d'objet
				} else if (context.nodeName.toLowerCase ()! == "objet") {

					// Capture l'ID de contexte, le paramètre en premier si nécessaire
					...
					i = groups.length;
					alors que je-- ) {
						groups [i] = "#" + nid + "" + toSelector (groupes [i]);
					}
					newSelector = groups.join (",");

					// Développer le contexte pour les sélecteurs de frères et soeurs
	...
	// Tous les autres
	return select (selector.replace (rtrim, "$ 1"), contexte, résultats, seed);
}

/ **
 * Créer des caches de valeur-clé de taille limitée
 * @returns {function (string, object)} Retourne les données de l'objet après les avoir stockées
 * nom de la propriété la chaîne (espace-suffixé) et (si le cache est plus grand que Expr.cacheLength)
...
function createCache () {
	...
	cache de fonction (clé, valeur) {
		// Utilise (touche + "") pour éviter la collision avec les propriétés du prototype natif (voir le numéro 157)
...
/ **
 * Marquez une fonction pour un usage spécial par Sizzle
...
/ **
 * Support des tests en utilisant un élément
 * @param {Fonction} fn Passé l'élément créé et retourne un résultat booléen
 * /
fonction assert (fn) {
...
/ **
 * Vérifie l'ordre des documents de deux frères et soeurs
 * @param {Element} a
 * @param {Element} b
 * @returns {Number} Retourne moins de 0 si a précède b, plus grand que 0 si a suit b
 * /
function siblingCheck (a, b) {
	...
	// Utiliser IE sourceIndex si disponible sur les deux noeuds
	if (diff) {
		return diff;
	}
...
function createInputPseudo (type) {
	fonction de retour (elem) {
		var nom = elem.nodeName.toLowCase ();
		return name === "entrée" && elem.type === type;
	}
}
...
function createDisabledPseudo (désactivé) {
	...
	// Connu: désactivé les faux positifs: fieldset [désactivé]> légende: nth-of-type (n + 2): peut-désactiver
	fonction de retour (elem) {
		...
		if ("form" dans elem) {
			...
			// Vérifier l'invalidité héritée sur les éléments pertinents non-désactivés:
			// * éléments listés associés à une forme dans un fieldset désactivé
			...
			if (elem.parentNode && elem.disabled === false) {
				...
				// Les éléments d'option sont renvoyés à un groupe d'opt parent, le cas échéant
				if ("label" dans elem) {
				...
				// Support: IE 6 - 11
				// Utilisez la propriété de raccourci isDisabled pour vérifier les ancêtres de groupes de champs désactivés
				return elem.isDisabled === désactivé ||

					// Où il n'y a pas isDisabled, vérifiez manuellement
					/ * jshint -W018 * /
		...
		// Essayez de supprimer les éléments qui ne peuvent pas être désactivés avant d'approuver la propriété désactivée.
		...
		return markFunction (fonction (graine, correspondances) {
			...
			alors que je-- ) {
				if (seed [(j = matchIndexes [i])]) {
					seed [j] =! (correspond à [j] = graine [j]);
				}
			}
		});
	});
 ...
 * @returns {Element | Object | Boolean} Le noeud d'entrée si acceptable, sinon une valeur falsifiée
 * /
function testContext (contexte) {
	return context && typeof context.getElementsByTagName! == "undefined" && contexte;
}

// Expose support vars pour plus de commodité
 ...
 * @param {Element | Object} [doc] Objet ou objet de document à utiliser pour définir le document
 * @returns {Object} Retourne le document courant
 * /
setDocument = Sizzle.setDocument = function (node) {
	var hasCompare, subWindow,
		doc = noeud? node.ownerDocument || noeud: preferredDoc;

	// Renvoie tôt si doc est invalide ou déjà sélectionné
	...
	// Accéder aux documents iframe après le déchargement génère des erreurs "permission denied" (jQuery # 13936)
	...
	// Vérifie si getElementsByTagName ("*") ne retourne que des éléments
	support.getElementsByTagName = assert (function (el) {
		el.appendChild (document.createComment (""));
		return! el.getElementsByTagName ("*"). longueur;
	});
	...
	// Support: IE <10
	// Vérifie si getElementById renvoie des éléments par leur nom
	// Les méthodes broken getElementById ne détectent pas les noms définis par programmation,
	...
	if (support.getById) {
		...
		Expr.find ["ID"] = fonction (id, contexte) {
			if (typeof context.getElementById! == "non défini" && documentIsHTML) {
		...
		Expr.find ["ID"] = fonction (id, contexte) {
			if (typeof context.getElementById! == "non défini" && documentIsHTML) {
				...
				if (elem) {
					...
					// Vérifie l'attribut id
					node = elem.getAttributeNode ("id");
					if (node ​​&& node.value === id) {
						return [elem];
					}

					// Replie sur getElementsByName
	...
	Expr.find ["TAG"] = support.getElementsByTagName?
		function (tag, contexte) {
			if (typeof context.getElementsByTagName! == "undefined") {
				return context.getElementsByTagName (tag);

			// Les noeuds DocumentFragment n'ont pas de gEBTN
			} else if (support.qsa) {
		...
		function (tag, contexte) {
			var elem,
				tmp = [],
				i = 0,
				// Par bonne coïncidence, un gEBTN (cassé) apparaît aussi sur les nœuds DocumentFragment
	...
	Expr.find ["CLASS"] = support.getElementsByClassName && function (className, context) {
		if (typeof context.getElementsByClassName! == "non défini" && documentIsHTML) {
	...
	// Support QSA et matchesSelector

	// matchesSelector (: active) signale false si vrai (IE9 / Opera 11.5)
	rbuggyMatches = [];

	// qSa (: focus) signale false si vrai (Chrome 21)
	// Nous autorisons cela à cause d'un bug dans IE8 / 9 qui génère une erreur
	// chaque fois que `document.activeElement` est accédé sur un iframe
	// Donc, nous autorisons: le focus de passer par QSA tout le temps pour éviter l'erreur IE
	// Voir https://bugs.jquery.com/ticket/13378
	rbuggyQSA = [];

	if ((support.qsa = rnative.test (document.querySelectorAll))) {
		// Construire une regex QSA
		// Stratégie Regex adoptée de Diego Perini
		assert (function (el) {
			// Select est réglé sur une chaîne vide à dessein
			// Ceci est pour tester le traitement de IE non explicitement
			// définir un attribut de contenu booléen,
			// puisque sa présence devrait être suffisante
			...
			// L'attribut de test doit être inconnu dans Opera mais "safe" pour WinRT
			// https://msdn.microsoft.com/en-us/library/ie/hh465388.aspx#attribute_section
			if (el.querySelectorAll ("[msallowcapture ^ = '']"). length) {
				rbuggyQSA.push ("[* ^ $] =" + espace blanc + "* (?: '' | \" \ ")");
			}
			...
			// Webkit / Opera -: cochée doit renvoyer les éléments d'option sélectionnés
			// http://www.w3.org/TR/2011/REC-css3-selectors-20110929/#checked
			// IE8 renvoie une erreur ici et ne verra pas les tests ultérieurs
			if (! el.querySelectorAll (": checked"). longueur) {
				rbuggyQSA.push (": checked");
			}

			// Support: Safari 8+, iOS 8+
			// https://bugs.webkit.org/show_bug.cgi?id=136851
			// Inlet "sélecteur # id sibling-combinator selector" échoue
		...
		assert (function (el) {
			el.innerHTML = "<a href='' disabled='disabled'> </a>" +
				"<select disabled = 'désactivé'> <option /> </ select>";

			// Support: Windows 8 Applications natives
			// Les attributs de type et de nom sont limités pendant l'affectation .innerHTML
			var input = document.createElement ("entrée");
			...
			if (el.querySelectorAll ("[nom = d]"). longueur) {
				rbuggyQSA.push ("nom" + espace blanc + "* [* ^ $ |! ~]? =");
			}

			// FF 3.5 -: enabled /: éléments désactivés et cachés (les éléments cachés sont toujours activés)
			// IE8 renvoie une erreur ici et ne verra pas les tests ultérieurs
			if (el.querySelectorAll (": enabled"). longueur! == 2) {
				rbuggyQSA.push (": enabled", ": disabled");
			}

			// Support: IE9-11 +
			// IE: le sélecteur désactivé ne récupère pas les enfants des fieldsets désactivés
			docElem.appendChild (el) .disabled = true;
			if (el.querySelectorAll (": disabled"). longueur! == 2) {
				rbuggyQSA.push (": enabled", ": disabled");
			}

			// Opera 10-11 ne lance pas sur les pseudos incorrects post-virgules
			el.querySelectorAll ("* ,: x");
			rbuggyQSA.push (",. *:");
		});
	}

	if ((support.matchesSelector = rnative.test ((correspond à = docElem.matches ||
		...
		assert (function (el) {
			...
			// Gecko ne fait pas d'erreur, retourne false à la place
			matches.call (el, "[s! = '']: x");
	...
	// Comme dans, un élément ne contient pas lui-même
	...
	fonction (a, b) {
		...
		// Indicateur pour le retrait des doublons
		if (a === b) {
			hasDuplicate = true;
			renvoie 0;
		}
		...
		if (comparez & 1 ||
			...
			// Choisissez le premier élément associé à notre document préféré
		...
		retour comparer et 4? -1: 1;
	}:
	fonction (a, b) {
		// Quitter tôt si les noeuds sont identiques
		if (a === b) {
			hasDuplicate = true;
			renvoie 0;
		}
		...
		// Les noeuds sans parent sont des documents ou déconnectés
		if (! aup ||! bup) {
		...
		// Si les noeuds sont frères et soeurs, nous pouvons faire une vérification rapide
		} else if (aup === bup) {
			return siblingCheck (a, b);
		}

		// Sinon, nous avons besoin de listes complètes de leurs ancêtres pour la comparaison
		cur = a;
		...
		// Descendez l'arbre à la recherche d'une divergence
		while (ap [i] === bp [i]) {
			i ++;
		}

		retour je?
			// Effectue un contrôle parental si les nœuds ont un ancêtre commun
			siblingCheck (ap [i], bp [i]):

			// Sinon, les noeuds de notre document trient d'abord
			ap [i] === preferredDoc? -1 :
			bp [i] === preferredDoc? 1 :
			0;
	}
...
Sizzle.matches = function (expr, éléments) {
...
Sizzle.matchesSelector = function (elem, expr) {
	...
	expr = rep.replace (rattributeQuotes, "= '$ 1']");
	...
	if (support.matchesSelector && documentIsHTML &&
		...
		essayez {
			...
			// matchesSelector d'IE 9 renvoie false sur les noeuds déconnectés
			if (ret || support.disconnectedMatch ||
					// De plus, les nœuds déconnectés sont dans un document
					// fragment dans IE 9
...
Sizzle.attr = function (elem, nom) {
	...
	var fn = Expr.attrHandle [name.toLowerCase ()],
		...
		val = fn && hasOwn.call (Expr.attrHandle, nom.àLowerCase ())?
			fn (elem, nom,! documentIsHTML):
			indéfini;

	return val! == non défini?
		val:
...
Sizzle.error = function (msg) {
	throw new Erreur ("Erreur de syntaxe, expression non reconnue:" + msg);
}
...
Sizzle.uniqueSort = fonction (résultats) {
	...
	// A moins que nous * sachions * nous pouvons détecter les doublons, assumer leur présence
	...
	if (hasDuplicate) {
		...
		tandis que (j--) {
			results.splice (duplique [j], 1);
		}
	}

	// Effacer l'entrée après le tri pour libérer des objets
...
/ **
 * Fonction utilitaire pour récupérer la valeur de texte d'un tableau de noeuds DOM
...
getText = Sizzle.getText = function (elem) {
	...
	if (! nodeType) {
		// S'il n'y a pas nodeType, ceci devrait être un tableau
		while ((node ​​= elem [i ++])) {
			// Ne pas trader les noeuds de commentaire
			ret + = getText (noeud);
		}
	} else if (nodeType === 1 || nodeType === 9 || nodeType === 11) {
		// Utilise textContent pour les éléments
	...
	// N'inclut pas les noeuds d'instruction de commentaire ou de traitement
...
Expr = Sizzle.selectors = {
	...
	preFilter: {
		"ATTR": function (match) {
			...
			// Déplacer la valeur donnée pour qu'elle corresponde à [3], qu'elle soit citée ou non
		...
		"CHILD": function (match) {
			...
			* /
			...
			if (match [1] .slice (0, 3) === "nième") {
				...
				// se souvient que false / true cast respectivement à 0/1
				match [4] = + (match [4]? match [5] + (match [6] || 1): 2 * (match [3] === "even" || match [3] === " impair "));
		...
		"PSEUDO": function (match) {
			...
			if (matchExpr ["CHILD"]. test (correspondance [0])) {
				return null;
			}
	...
	filtre: {
		...
		"CLASS": function (className) {
			...
			motif de retour ||
				(pattern = new RegExp ("(^ |" + espace + ")" + className + "(" + espace + "| $)")) &&
		...
		"ATTR": fonction (nom, opérateur, vérification) {
			fonction de retour (elem) {
				...
				retour opérateur === "="? résultat === vérifier:
					opérateur === "! ="? résultat! == vérifier:
					opérateur === "^ ="? check && result.indexOf (cochez) === 0:
					opérateur === "* ="? check && result.indexOf (cocher)> -1:
					opérateur === "$ ="? vérifiez && result.slice (-check.length) === vérifiez:
					opérateur === "~ ="? ("" + result.replace (rwhitespace, "") + "") .indexOf (vérifier)> -1:
					opérateur === "| ="? résultat === vérifier || result.slice (0, check.length + 1) === cocher + "-":
					faux;
			}
		},
			...
			retourner en premier === 1 && last === 0?
				...
				function (elem, context, xml) {
					cache var, uniqueCache, outerCache, node, nodeIndex, start,
						...
						name = ofType && elem.nodeName.toLowCase (),
						useCache =! xml &&! deType,
						diff = false;
					...
					si (parent) {
						...
						if (transfère && useCache) {

							// Recherche `elem` d'un index précédemment mis en cache
							...
							// Défense contre les attroupés clonés (jQuery gh-1709)
							...
							node = nodeIndex && parent.childNodes [nœudIndex];

							while ((node ​​= ++ nodeIndex && noeud && noeud [dir] ||

								// Repli sur la recherche d'un «elem» dès le début
								(diff = nodeIndex = 0) || start.pop ())) {

								// Quand trouvé, cache les index sur `parent` et casse
						...
						diff - = dernier;
						return diff === premier || (diff% first === 0 && diff / premier> = 0);
					}
				}
		},

		"PSEUDO": function (pseudo, argument) {
			// Les noms de pseudo-classe sont insensibles à la casse
			// http://www.w3.org/TR/selectors/#pseudo-classes
			// Prioriser en fonction de la casse si des pseudos personnalisés sont ajoutés avec des majuscules
			// Rappelez-vous que setFilters hérite des pseudos
			var args,
				fn = Expr.pseudos [pseudo] || Expr.setFilters [pseudo.toLowerCase ()] ||
					Sizzle.error ("pseudo non supporté:" + pseudo);

			// L'utilisateur peut utiliser createPseudo pour indiquer que
			// les arguments sont nécessaires pour créer la fonction de filtre
			// juste comme Sizzle
			if (fn [expando]) {
				return fn (argument);
			}

			// Maintenir le support pour les anciennes signatures
			if (fn.length> 1) {
	...
	pseudos: {
		...
		"not": markFunction (fonction (sélecteur) {
			// Ajuste le sélecteur passé à la compilation
			...
			retour matcher [expando]?
				markFunction (fonction (graine, correspondances, contexte, xml) {
					...
					// Correspond à des éléments sans correspondance avec `matcher`
					alors que je-- ) {
						if ((elem = inégalé [i])) {
				...
				function (elem, context, xml) {
					...
					// Ne conserve pas l'élément (problème # 299)
					entrée [0] = null;
					return! results.pop ();
				}
		}),
		...
		"contains": markFunction (function (texte) {
		...
		// ou commençant par l'identifiant C suivi immédiatement de "-".
		...
		"enabled": createDisabledPseudo (faux),
		"disabled": createDisabledPseudo (vrai),

		"checked": function (elem) {
			// En CSS3,: checked doit renvoyer les éléments cochés et sélectionnés
			// http://www.w3.org/TR/2011/REC-css3-selectors-20110929/#checked
			var nodeName = elem.nodeName.toLowCase ();
			return (nodeName === "input" && !! elem.checked) || (nodeName === "option" && !! elem.selected);
		},

		"selected": function (elem) {
			// Accéder à cette propriété fait selected-by-default
		...
		"empty": function (elem) {
			...
			//: vide est annulé par l'élément (1) ou les nœuds de contenu (texte: 3; cdata: 4; entité réf: 5),
